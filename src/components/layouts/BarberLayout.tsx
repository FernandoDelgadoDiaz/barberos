...
export function BarberLayout() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--primary, #1a1a1a)' }}>
      <header className="barber-header" style={{ background: 'var(--primary, #1a1a1a)', borderBottom: '1px solid var(--border, #383838)', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span className="hide-mobile" style={{ background: 'var(--card, #2a2a2a)', border: '1px solid var(--border, #383838)', borderRadius: '100px', padding: '4px 12px', fontSize: '10px', color: 'var(--text-tertiary, #777)', letterSpacing: '1.5px', textTransform: 'uppercase' }}>barbero</span>
      ...
      <nav className="barber-nav" style={{ background: 'var(--primary, #1a1a1a)', borderBottom: '1px solid var(--border, #383838)', display: 'flex', padding: '0 20px' }}>
        {[{ to: '/barber/dashboard', label: 'Mi día' }, { to: '/barber/summary', label: 'Resumen' }].map(({ to, label }) => (
          <NavLink key={to} to={to} className="barber-nav-link" style={({ isActive }) => ({ padding: '12px 16px', fontSize: '13px', fontWeight: 500, color: isActive ? 'var(--secondary, #C8A97E)' : 'var(--text-dim, #555)', borderBottom: isActive ? '2px solid var(--secondary, #C8A97E)' : '2px solid transparent', textDecoration: 'none', letterSpacing: '0.3px' })}>
            {label}
          </NavLink>
        ))}
      </nav>
      <main className="barber-main" style={{ padding: '20px' }}>
        <Outlet />
      </main>
    </div>
  )
