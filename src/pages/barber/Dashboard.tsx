...
export function Dashboard() {
  ...
  return (
    <div style={{ maxWidth: '430px', margin: '0 auto', padding: '0 16px', paddingBottom: 'calc(100px + env(safe-area-inset-bottom))', boxSizing: 'border-box', overflowX: 'hidden' }}>
            ...
            {wizardStep === 2 && (
              <>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button
                    onClick={() => { setWizardPaymentMethod('efectivo'); setWizardTipPaymentMethod('efectivo'); setWizardOthersPaymentMethod('efectivo'); setWizardStep(tenant.tip_policy?.enabled === false ? (tenant.product_policy?.enabled === false ? 5 : 4) : 3) }}
                  ...
                  <button
                    onClick={() => { setWizardPaymentMethod('transferencia'); setWizardTipPaymentMethod('transferencia'); setWizardOthersPaymentMethod('transferencia'); setWizardStep(tenant.tip_policy?.enabled === false ? (tenant.product_policy?.enabled === false ? 5 : 4) : 3) }}
              ...
              <>
                ...
                {!wizardTipEnabled ? (
                  <div style={{ display: 'flex', gap: '12px' }}>
                    ...
                    <button onClick={() => { setWizardTip(''); setWizardStep(tenant.product_policy?.enabled === false ? 5 : 4) }} style={{ flex: 1, padding: '16px', borderRadius: '12px', border: 'none', background: C.blue, color: '#fff', fontFamily: fontBody, fontWeight: 600, fontSize: '14px', cursor: 'pointer' }}>
                      No, continuar →
                    </button>
                  </div>
                ) : (
                  <>
                    ...
                    <button onClick={() => setWizardStep(tenant.product_policy?.enabled === false ? 5 : 4)} style={{ width: '100%', height: '48px', background: C.blue, color: '#fff', border: 'none', borderRadius: '12px', fontFamily: fontBody, fontWeight: 600, fontSize: '15px', cursor: 'pointer' }}>
                      Siguiente →
                    </button>
                  </>
                )}
